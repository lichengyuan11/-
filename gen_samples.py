...
def gen_samples():
    ...
    with open('data/climate_data.csv', 'r') as csvfile:
    ...
    # day_num = 31
    t_num = day_num*288
    bus_load2 = [i for i in range(18, 34)]
    bus_load1 = [i for i in range(2, 18)]

    bus_solar = [7, 8, 24, 25, 30, 32]
    bus_wind = [4, 14, 29, 31]
    p_solar = np.zeros(33)
    p_solar[bus_solar] = [180, 250, 250, 250, 150, 180]
    p_wind = np.zeros(33)
    p_wind[bus_wind] = [120, 120, 120, 150]
    ...
    for it in range(len(t)):
        ...
        for b in bus_load1:
            mpc["bus"][b-1, bus_map["pd"]-1] = mpc["bus"][b-1, bus_map["pd"]-1]*load1*data[it, b + 8]
            mpc["bus"][b-1, bus_map["qd"]-1] = mpc["bus"][b-1, bus_map["qd"]-1]*load1*data[it, b + 8]
        
        for b in bus_load2:
            mpc["bus"][b-1, bus_map["pd"]-1] = mpc["bus"][b-1, bus_map["pd"]-1]*load2*data[it, b + 8]
            mpc["bus"][b-1, bus_map["qd"]-1] = mpc["bus"][b-1, bus_map["qd"]-1]*load2*data[it, b + 8]
...
# BBM = cal_BBM(mpc)
raw = gen_samples()
I_rate = np.zeros([32, raw["e"].shape[-1]])
...
heavy_rate[0] = np.sum(I_rate>0.8, axis=0)/32
